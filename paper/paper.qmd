---
title: "A 1% Increase in The Proportion of Spending on Facilities Decreases Graduation Rates by 0.2%: An Analysis on Ontario School Districts and Related Predictors"
author: Andrew Goh
...
ggplot(analysis_data, aes(x = percentage_spent_on_facilities)) +
  geom_histogram(fill = "#2c7bb6", color = "white", alpha = 0.8) +
  labs(
    x = "Percentage Spent on Facilities",
    y = "Number of School Boards"
  ) +
  theme_minimal()
```
...
ggplot(analysis_data, aes(x = percent_no_degree)) +
  geom_histogram(fill = "#2c7bb6", color = "white", alpha = 0.8) +
  labs(
    x = "Percentage of Students Without Parents Holding a Post-Secondary Degree",
    y = "Number of School Boards"
  ) +
  theme_minimal()
```
...
ggplot(analysis_data, aes(x = percent_low_income)) +
  geom_histogram(fill = "#2c7bb6", color = "white", alpha = 0.8) +
  labs(
    x = "Percentage of Students in Low Income Families",
    y = "Number of School Boards"
  ) +
  theme_minimal()
```
...
#| message: false
#| warning: false
ggplot(data_scaled, aes(
  x = percentage_spent_on_facilities,
  ...
  labs(
    x = "Percentage Spent on Facilities (Standardized)",
...
ggplot(data_scaled, aes(
  ...
  labs(
    x = "Expenses per Student (Standardized)",
    y = "Graduation Rate (Standardized)"
  ) +
  theme_minimal()
```
...
ggplot(data_scaled, aes(
  ...
  labs(
    x = "Total Enrolment (Standardized)",
    y = "Graduation Rate (Standardized)"
  ) +
  theme_minimal()
```
...
ggplot(data_scaled, aes(
  ...
  labs(
    x = "Predicted Graduation Rate (Standardized)",
...
The non-linear relationship between per-student expenses and graduation rates is particularly noteworthy. The U-shaped relationship we observed suggests that simply increasing funding may not always lead to better outcomes. This finding aligns with previous research suggesting that the effectiveness of educational spending depends heavily on how resources are allocated [@educationspending]. School boards might benefit from examining the spending patterns of high-performing boards that operate in the "efficient" region of this curve.
...
```{r}
#| label: fig-appendix-cook
#| fig-cap: "Cook's Distance Plot"
#| echo: false
#| message: false
#| warning: false

plot(cooks.distance(poly_model),
  type = "h",
...
abline(
  h = 4 / length(cooks.distance(poly_model)),
  col = "red",
  lty = 2
)
```
...
@fig-appendix-cook shows Cook's distance for each observation, helping identify potentially influential data points. The dashed red line indicates a commonly used threshold of 4/n for identifying influential observations.
