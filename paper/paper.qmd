---
title: "Facilities Spending and Socioeconomic Factors Show Strong Correlation with Ontario School District Graduation Rates"
author: Andrew Goh
...
@tbl-summary presents an overview of the summary statistics of our key variables across Ontario school boards, and @tbl-snapshot presents a snapshot of the data itself.
...
summary_stats %>%
  ...
  tab_footnote(
    footnote = "Total Expenses shown in billions ($), Enrollment in thousands, percentages as raw values, expenses per student in dollars.",
...
```{r}
#| label: tbl-snapshot
#| tbl-cap: "Sample of First Five Rows from Ontario School Board Dataset"
#| echo: false
#| warning: false
#| message: false
...
knitr::kable(
  analysis_data |>
    select(`Board Number`,
           ...
           `Total Expenses`) |>
    slice(1:5) |>
    mutate(Region = str_replace(Region, "Region ", "")),  # Remove the word "Region"
...
The percentage of students from low-income households ranges from 10.35% to 25.92%, with a mean of 15.77%. This variable provides insight into the socioeconomic composition of each school board's student population.
...
Our final and chosen model is a polynomial regression of degree 2:
...
1. **Non-linear Relationships**: Initial exploratory data analysis revealed non-linear relationships between several predictors and graduation rates, particularly for expenses per student and total enrollment.

2. **Model Comparison**: Comparing Mean Squared Error (MSE) across models:
...
```{r}
#| label: fig-residuals
#| fig-cap: Residual Diagnostics for Polynomial Regression Model
#| echo: false
#| message: false
#| warning: false
...
The percentage of households without post-secondary degrees shows the strongest relationship with graduation rates (β = -3.627, p = 0.003). This linear relationship suggests that for every standard deviation increase in the percentage of households without degrees, graduation rates decrease by approximately 3.6 percentage points, holding other factors constant.
...
Per-student expenses show a significant quadratic relationship (β = 3.444, p = 0.013), as shown in @fig-expenses. The U-shaped relationship suggests that while initial increases in per-student spending may be associated with lower graduation rates, there appears to be a threshold beyond which additional spending becomes positively associated with graduation rates.
...
#| message: false
#| warning: false
ggplot(data_scaled, aes(x = `Total Expenses`, 
                        y = `Four Year Graduation Rate`)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", 
              formula = y ~ poly(x, 2), 
              se = TRUE) +
  labs(x = "Total Expenses (Standardized)",
       y = "Graduation Rate (Standardized)") +
  theme_minimal()
...
ggplot(data_scaled, aes(x = `Total Enrolment`, 
  ...
  geom_smooth(method = "lm", 
              formula = y ~ poly(x, 2), 
              se = TRUE) +
  labs(x = "Total Enrollment (Standardized)",
       y = "Graduation Rate (Standardized)") +
  theme_minimal()
```
...
@fig-fit shows the relationship between predicted and actual graduation rates. The model explains approximately 40.4% of the variance in graduation rates (R² = 0.404), suggesting that while our identified factors are important, there remain other unobserved variables that influence graduation rates.
...
https://data.ontario.ca/dataset/school-board-achievements-and-progress/resource/9d81dc27-32ef-4864-94b5-f09950d00c72

https://data.ontario.ca/dataset/school-board-financial-reports-estimates-revised-estimates-and-financial-statements/resource/56598892-c180-4f0d-bde5-63f971d6ef9a
